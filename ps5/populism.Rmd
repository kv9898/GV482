...
need <- c('tidyverse','haven','modelsummary',"TAM","fixest","estimatr") # list packages
...
fml4 <- paste(c(fml3, paste(grep("l_sh_pop_",names(data),value=T), collapse = " + ")), collapse = " + ")
...
glance_custom.fixest <- function(x, ...) {
  data.frame(
    "F" = round(fitstat(x,"F")$f$stat,1)
  )}

gof_map <- list(list("raw" = "F", "clean" = "$F$", "fmt" = 1),
...
[The first stage results are displayed in Table \@ref(tab:q6). ]{style="color: blue;"}
