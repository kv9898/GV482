...
author: "Dianyi Yang <br>GV482^[Questions? Email [l.s.bosshart@lse.ac.uk](mailto:l.s.bosshart@lse.ac.uk).<br>R adaption of Stephane Wolton's STATA problem set implemented by [Felix Wortmann Callejón](https://www.wortmanncallejon.de).]"
...
```{r tabletwo, echo = F}
# You can insert the code to generate your answer here
des |> kbl(digits=3, caption="Racial composition of the dataset by year") |>
  kable_classic() |>
...
d <- feglm(useanyforce ~ sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers + 
             #foundweapon + foundcontraorweap + 
             carrysuspobj + fitreldescr + prepforcrime + lookout + crimattire + appdrugtrans + suspmovements + engageviolcrime + concealsuspobj + othersuspbehav, data=data, cluster=~pct, family="binomial")
e <- feglm(useanyforce ~ sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers + 
             #foundweapon + foundcontraorweap + 
             carrysuspobj + fitreldescr + prepforcrime + lookout + crimattire + appdrugtrans + suspmovements + engageviolcrime + concealsuspobj + othersuspbehav|pct+year, data=data, cluster=~pct, family="binomial")

mods <- list("No controls" = a,
     "+ civilian demographics" = b,
     "+ encounter characteristics" = c,
     "+ civilian behavior" = d,
     "+ precint and year FE" = e)

coef_map <- c("sblack" = "Black", "shisp" = "Hispanic", "sasian" = "Asian", "sother" = "Other Race")
logit <- msummary(mods, coef_map=coef_map, exp=TRUE, stars=FALSE, 
         shape = model + statistic ~ term,
         title="Logit Regressions of Use of Force on Race")
save(logit, file="logit.RData")
```

```{r panelAFryertab, eval=T}
load("logit.RData")
logit
```
...
d <- feols(useanyforce ~ sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers + 
             #foundweapon + foundcontraorweap + 
             carrysuspobj + fitreldescr + prepforcrime + lookout + crimattire + appdrugtrans + suspmovements + engageviolcrime + concealsuspobj + othersuspbehav, data=data, cluster=~pct,)
...
coef_map <- c("sblack" = "Black", "shisp" = "Hispanic", "sasian" = "Asian", "sother" = "Other Race")
panelAOLS <- msummary(mods, coef_map=coef_map, stars=FALSE, 
         shape = model + statistic ~ term,
         title="OLS Regressions of Use of Force on Race")
save(panelAOLS, file="panelAOLS.RData")
```


```{r panelAOLStab}
load("panelAOLS.RData")
panelAOLS
```
...
-   First term: difference in observed violence

-   Second term: proportion of stops due to discrimination

-   statistical discrimination in beliefs about violence levels.
...
```{r criminalviolence, eval=F}
a <- feols(useanyforce ~ foundweapon + foundcontraorweap, data=data, cluster=~pct)
b <- feols(useanyforce ~ foundweapon + foundcontraorweap + sblack + shisp + sasian + sother + smale + sage + sage^2, data=data, cluster=~pct)
c <- feols(useanyforce ~ foundweapon + foundcontraorweap + sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers, data=data, cluster=~pct)
d <- feols(useanyforce ~ foundweapon + foundcontraorweap + sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers + carrysuspobj + fitreldescr + prepforcrime + lookout + crimattire + appdrugtrans + suspmovements + engageviolcrime + concealsuspobj + othersuspbehav, data=data, cluster=~pct,)
e <- feols(useanyforce ~ foundweapon + foundcontraorweap + sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers + carrysuspobj + fitreldescr + prepforcrime + lookout + crimattire + appdrugtrans + suspmovements + engageviolcrime + concealsuspobj + othersuspbehav|pct+ year, data=data, cluster=~pct,)

mods <- list("No controls" = a,
     "+ civilian demographics" = b,
     "+ encounter characteristics" = c,
     "+ civilian behavior" = d,
     "+ precinct and year fixed effects" = e)

coef_map <- c("foundweapon" = "Weapon found",
              "foundcontraorweap" = "Contraband found",
              "sblack" = "Black", 
              "shisp" = "Hispanic", 
              "sasian" = "Asian", 
              "sother" = "Other Race")

criminalviolence <- msummary(mods, coef_map=coef_map, stars=T, 
         shape = model + statistic ~ term,
         title="OLS Regressions of violence on weapon or contraband found")
save(criminalviolence, file="criminalviolence.RData")
```

```{r criminalviolencetab}
load("criminalviolence.RData")
criminalviolence
```
...
Criminals are more prone to trigger violence (under our assumptions)
...
```{r ambigoussignal, eval=F}
a <- feols(useanyforce ~ sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers|pct+ year, data=data, cluster=~pct,
           subset=~concealsuspobj==1)

b <- feols(useanyforce ~ sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers|pct+ year, data=data, cluster=~pct,
           subset=~crimattire==1)

c <- feols(useanyforce ~ sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers|pct+ year, data=data, cluster=~pct,
           subset=~suspmovements==1)

d <- feols(useanyforce ~ sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers|pct+ year, data=data, cluster=~pct,
           subset=~engageviolcrime==1)

e <- feols(useanyforce ~ sblack + shisp + sasian + sother + smale + sage + sage^2 + indoors + daytime + hicrimearea + hicrimetime + hicrimeareaXtime + inuniform + idphoto + idverbal + idrefused + idother + withothers|pct+ year, data=data, cluster=~pct,
           subset=~prepforcrime==1)

mods <- list("Suspicious object" = a,
             "Criminal attire" = b,
             "Furtive movements" = c,
             "Violent crime" = d,
             "Preparing for crime" = e)

coef_map <- c("sblack" = "Black", 
              "shisp" = "Hispanic", 
              "sasian" = "Asian", 
              "sother" = "Other Race")

ambigoussignal <- msummary(mods, coef_map=coef_map, stars=T, 
         shape = model + statistic ~ term,
         title="OLS Regressions of violence on race for each signal")

save(ambigoussignal, file="ambigoussignal.RData")
```

```{r ambigoussignaltab}
load("ambigoussignal.RData")
ambigoussignal
```
...
The issue with this approach is that, the ambiguous signals might lead to more discrimination, as our theory predicts, whereas the seemingly discriminant discriminant signals might lead to less discrimination if they are unambiguous.
