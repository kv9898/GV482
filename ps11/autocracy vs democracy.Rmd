...
need <- c('tidyverse','haven','modelsummary',"fixest","estimatr","kableExtra",
          "arsenal","car","ggfixest","plm") # list packages
...
```{r ab}
data_plm <- pdata.frame(data, index = c("wbcode2", "year"))
within0 <- feols(y ~ dem|wbcode2+year, data = data)
ab1 <- pgmm(y ~ dem + lag(y,1)|lag(y,2:50)+lag(dem,1:50),
            collapse=F,
                 effect="twoways",
                 model="onestep",
                            data=data_plm)
ab_total_attack_geo <- pgmm(as.formula(paste0(outcome,"lag(PopulistTotal,1)+",
                                              homeattacks,geoattacks,controls,
                                              "+Year|lag(PopulistTotal,2)")), 
                            effect="individual",
                            weights=data_ab_pdata$weight,
                            data=data_ab_pdata)
within2 <- feols(y ~ dem + l(y,1) + l(y,2)|wbcode2+year, data = data, panel.id=~wbcode2+year)
within3 <- feols(y ~ dem + l(y,1) + l(y,2) + l(y,3) + l(y,4)|wbcode2+year, data = data, panel.id=~wbcode2+year)
within4 <- feols(y ~ dem + l(y,1:8)|wbcode2+year, data = data, panel.id=~wbcode2+year)
